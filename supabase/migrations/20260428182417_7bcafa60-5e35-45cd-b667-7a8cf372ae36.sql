-- Corrige _regra_vigente: filtra por equipamento específico OU regra geral (NULL)
CREATE OR REPLACE FUNCTION public._regra_vigente(_contrato_id uuid, _equipamento_id uuid, _tipo regra_tipo, _periodo_inicio date, _periodo_fim date)
 RETURNS jsonb
 LANGUAGE sql
...
AS $function$
  ...
  WHERE r.contrato_id = _contrato_id
    AND r.tipo = _tipo
    AND r.ativa = true
    -- Apenas regras do equipamento OU regras gerais (sem equipamento)
    AND (r.equipamento_id = _equipamento_id OR r.equipamento_id IS NULL)
    -- Sobreposição com o período da medição
    AND r.vigencia_inicio <= _periodo_fim
    AND (r.vigencia_fim IS NULL OR r.vigencia_fim >= _periodo_inicio)
  ORDER BY
    -- Específica do equipamento sempre antes da geral
    (r.equipamento_id = _equipamento_id) DESC,
    r.vigencia_inicio DESC
  LIMIT 1;
$function$;

-- Versão antiga (com _data) removida — substituída pela nova assinatura
DROP FUNCTION IF EXISTS public._regra_vigente(uuid, uuid, regra_tipo, date);

-- Atualiza _calc_item_com_regras para passar período da medição
...
AS $function$
DECLARE
  it record; med record; ctr record; eqid uuid;
...
BEGIN
  ...
  -- Período de avaliação: período do item OU da medição
  ...
  r_vh    := public._regra_vigente(ctr.id, eqid, 'valor_hora', p_ini, p_fim);
  r_gar   := public._regra_vigente(ctr.id, eqid, 'garantia_minima', p_ini, p_fim);
  r_mec   := public._regra_vigente(ctr.id, eqid, 'desconto_horas_mecanicas', p_ini, p_fim);
  r_chuv  := public._regra_vigente(ctr.id, eqid, 'periodo_chuvoso', p_ini, p_fim);
  r_exc   := public._regra_vigente(ctr.id, eqid, 'excecao_chuvoso', p_ini, p_fim);
  r_dm    := public._regra_vigente(ctr.id, eqid, 'desconto_manual', p_ini, p_fim);
  r_compl := public._regra_vigente(ctr.id, eqid, 'complementar', p_ini, p_fim);
  r_pers  := public._regra_vigente(ctr.id, eqid, 'regra_personalizada', p_ini, p_fim);

  IF r_vh IS NOT NULL THEN
    v_valor_hora := COALESCE((r_vh->'parametros'->>'valor')::numeric, it.valor_hora);
    aplicadas := aplicadas || jsonb_build_object('tipo','valor_hora','origem',CASE WHEN (r_vh->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'valor', v_valor_hora,'regra_id', r_vh->>'id');
  ELSE
  ...
  IF r_gar IS NOT NULL THEN
    ...
    aplicadas := aplicadas || jsonb_build_object('tipo','garantia_minima','origem',CASE WHEN (r_gar->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'horas', v_garantia,'regra_id', r_gar->>'id');
  ELSE
  ...
  IF r_mec IS NOT NULL THEN
    ...
    aplicadas := aplicadas || jsonb_build_object('tipo','desconto_horas_mecanicas','origem',CASE WHEN (r_mec->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'aplicar', desconta_mec,'regra_id', r_mec->>'id');
  END IF;
  ...
  IF r_chuv IS NOT NULL THEN
    ...
    aplicadas := aplicadas || jsonb_build_object('tipo','periodo_chuvoso','origem',CASE WHEN (r_chuv->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'modo', modo_chuvoso,'regra_id', r_chuv->>'id');
  END IF;
  IF r_exc IS NOT NULL AND it.horas_excecao_chuvoso > 0 THEN
    modo_chuvoso := 'normal';
    aplicadas := aplicadas || jsonb_build_object('tipo','excecao_chuvoso','origem',CASE WHEN (r_exc->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'modo','normal','regra_id', r_exc->>'id');
  END IF;
  ...
  IF r_compl IS NOT NULL THEN
    ...
    aplicadas := aplicadas || jsonb_build_object('tipo','complementar','origem',CASE WHEN (r_compl->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'valor', v_compl,'regra_id', r_compl->>'id');
  END IF;
  ...
  IF r_dm IS NOT NULL THEN
    ...
    aplicadas := aplicadas || jsonb_build_object('tipo','desconto_manual','origem',CASE WHEN (r_dm->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,'valor', v_desc,'regra_id', r_dm->>'id');
  END IF;

  IF r_pers IS NOT NULL THEN
    aplicadas := aplicadas || jsonb_build_object('tipo','regra_personalizada','origem',CASE WHEN (r_pers->>'equipamento_id') IS NOT NULL THEN 'equipamento' ELSE 'contrato' END,
...
-- simular_regras_medicao: incluir info do equipamento legível e estatísticas de cobertura
...
BEGIN
  ...
  SELECT count(*) INTO v_regras_aplicaveis FROM public.contrato_regras
    WHERE contrato_id = v_ctr.id AND ativa = true
  ...
  -- Alertas: regras específicas de equipamentos que não estão na medição
  FOR r IN
    SELECT cr.*, e.serie AS eq_serie, e.tag AS eq_tag, e.tipo AS eq_tipo, e.modelo AS eq_modelo
  ...
  LOOP
    ...
    IF eq_match_count = 0 THEN
      v_alertas := v_alertas || jsonb_build_object(
        'tipo', 'regra_sem_equipamento',
        'regra_id', r.id,
        'regra_tipo', r.tipo,
        'equipamento_serie', r.eq_serie,
        'equipamento_tag', r.eq_tag,
